...
def get_audio_generator(args):
    ...
    if args.mode == "mic":
        ...
        print("[*] Recording from microphone... (Ctrl+C to stop)")
        ...
        async def file_gen():
            ...
            if (
                ...
                print(
                    "[!] Warning: File format is not 16k mono 16bit. Server might need resampling."
                )
...
async def start_client(args):
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            # 1. Send Config
            config = {"sample_rate": RATE, "encoding": "pcm_16", "language": "zh"}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["mic", "file"],
        default="mic",
        help="Audio source: 'mic' or 'file'",
    )
    parser.add_argument("--path", type=str, help="Path to .wav file (if mode is file)")
    parser.add_argument(
        "--session", type=str, default="test_room", help="Session ID (Room)"
    )
